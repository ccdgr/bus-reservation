...
### 2. 接口级削峰填谷 (Channel + Worker Pool)
针对下单等极高并发接口，摒弃直接阻塞等待数据库响应的传统做法。引入 Go 原生 Channel 作为缓冲队列（限流降级），并启动固定数量的 Worker 协程池异步消费请求。配合 HTTP `select` 超时控制，完美保护底层 MySQL 连接池不被瞬间击垮。

### 3. 缓存击穿防护 (Singleflight)
针对热门班次的并发查询，引入 `singleflight`。在缓存失效时，合并重复的数据库读请求，确保数据库压力不因并发激增而崩溃。

### 4. 异步解耦与延时取消 (RabbitMQ)
- **异步流转**: 用户点击预定后，系统完成前置校验与扣减后直接返回，具体业务异步流转，大幅提升响应速度。
- **延时取消**: 利用 **Dead Letter Exchange (DLX)** 实现延时队列。订单创建后自动进入 15 分钟倒计时，若超时未支付，系统将自动取消订单并归还库存。

### 5. 严密的业务规则防线
- **限购策略**: 核心链路强制校验，同一用户对同一班次仅允许保留一个活跃订单（禁止重复购买与黄牛占座）。
- **订单状态机**: 定义了 `待支付`、`待核验`、`已取消`、`已过期`、`已核验` 五大状态。通过严格的状态机校验确保业务流转的安全性和一致性。
