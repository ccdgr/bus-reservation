...
func NewRouter(engine *gin.Engine, userUsecase domain.UserUsecase, busUsecase domain.BusUsecase, orderUsecase domain.OrderUsecase, aliClient *alipay.Client, jwtSecret string) {
	authMiddleware := AuthMiddleware(jwtSecret)

	// Public routes
	publicV1 := engine.Group("/api/v1")
	{
		RegisterPublicOrderHandler(publicV1.Group("/payments"), orderUsecase, aliClient)
	}
